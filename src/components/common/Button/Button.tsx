...
export function BorderedButton(buttonProps: ButtonProps) {
  ...
  return (
    <button
      {...rest}
      className={cls`self-center rounded-md border-0.5 border-sky-500 py-2 px-10 text-sky-500 ${className}`}>
      {children}
    </button>
  );
}
...
export function SolidButton(buttonProps: ButtonProps) {
  ...
  return (
    <button
      {...rest}
      className={cls`self-center rounded-md bg-sky-500 py-2 px-10 text-white ${className}`}>
      {children}
    </button>
  );
}
